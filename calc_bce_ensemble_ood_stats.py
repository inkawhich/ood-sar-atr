...
class ResNetBCEEnsemble(nn.Module):
    def __init__(self, ckpt_prefix):
        ...
        for i in range(self.num_classes):
            self.nets.append(
                models.resnet18(num_classes=1,
                                drop_prob=dropout,
                                cls_method='ilr').to(device))
    ...
    def forward(self, x):
        ...
        for i in range(len(self.nets)):
            out = self.nets[i](x)
            outs = torch.cat((outs, out.view(out.size(0), 1)), 1)
        return outs
...
net = ResNetBCEEnsemble(CKPT_BASE)
net.eval()

clean_acc, clean_loss = helpers.test_model(
    net, device, ID_testloader, MEAN, STD)
print("Clean Acc: {}".format(clean_acc))
