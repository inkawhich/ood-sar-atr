...
#   - IPP = boolean for whether or not to do the input preprocessing step
#   - eps = epsilon of perturbation if IPP is set
def generate_ood_scores_MAHALANOBIS(base_model, loader, activation_dictionary, layer_names_list, layer_means_dict, layer_precisions_dict, norm_mean, norm_std, IPP=False, eps=0.):
	mahalanobis_scores = []
			...
			with torch.no_grad():
				...
				for lyr in layer_names_list:
					...
					if len(act.shape) == 4: # If activations are from a conv layer must average out spatial dims to get [N,C] shaped representation
						...
						sample_feature = act
					final_lyr_scores += compute_mahalanobis_score_batch(sample_feature, layer_means_dict[lyr].clone().detach(), layer_precisions_dict[lyr].clone().detach()) # should be N scores, one for each img in batch	
